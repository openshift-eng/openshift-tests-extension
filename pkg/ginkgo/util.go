...
// sourceDir is set at build time to filter out the fully qualified path from
// ginkgo's code locations, and used to instead give a relative path.
...
func BuildExtensionTestSpecsFromOpenShiftGinkgoSuite(selectFns ...ext.SelectFunction) (ext.ExtensionTestSpecs, error) {
	...
	ginkgo.GetSuite().WalkTests(func(name string, spec types.TestSpec) {
		...
		for _, cl := range spec.CodeLocations() {
			codeLocations = append(codeLocations, strings.TrimPrefix(cl.String(), sourceDir))
		}
	...
	// some reason, you must include vendored specs, you can opt-in directly by supplying your own SelectFunctions.
	if len(selectFns) == 0 {
